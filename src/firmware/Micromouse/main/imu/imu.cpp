...
#include "driver/i2c_master.h" // API I2C do ESP-IDF v6+
#include "i2c_manager.hpp"
#include "../pins.hpp"

// O bloco extern "C" evita name mangling da biblioteca em C.
extern "C" {
#include "mpu9250.h"           // Header da biblioteca truita/mpu9250 em C
}
#include <stdio.h>
...
static void salvar_calibracao_na_nvs() {
    ...
    // Grava a struct inteira como blob para reduzir tempo e desgaste da flash.
    if (nvs_set_blob(nvs_handle, NVS_BLOB_KEY, &calibracao_atual, sizeof(CalibracaoIMU)) == ESP_OK) {
        nvs_commit(nvs_handle); // Efetiva a gravacao no chip.
...
static bool carregar_calibracao_da_nvs() {
    ...
    // Le o blob direto para a struct.
...
// =============================================================================
//  Inicializacao e leitura do sensor
// =============================================================================
...
bool imu_init() {
    ...
    // Configuracao logica do MPU9250.
    ...
    // Vincula o driver ao barramento I2C.
...
bool imu_update() {
    ...
    // Faz a leitura I2C. Retorna erro se o sensor falhar durante a execucao.
    if (mpu9250_update(&mpu_device) != ESP_OK) {
        return false;
    }

    // Aplica os bias calculados na calibracao.
    ...
    dados_atuais.timestamp_ms = esp_timer_get_time() / 1000; // Tempo desde o boot em ms.
...
bool imu_calibrar_gyro() {
    ...
    // Acumula amostras para estimar o bias.
    for (int i = 0; i < amostras; i++) {
        ...
        vTaskDelay(pdMS_TO_TICKS(5)); // Cede CPU ao RTOS.
    }

    // Calcula a media do erro em repouso.
...
void imu_processar_serial() {
    // Entrada do handler de terminal UART (POSIX select).
}