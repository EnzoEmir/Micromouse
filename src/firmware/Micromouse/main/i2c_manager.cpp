...
bool i2c_manager_unregister_device(uint8_t address) {
    if (!i2c_manager_valid_address(address)) return false;
    if (!g_devices[address]) return true;
    esp_err_t err = i2c_master_bus_rm_device(g_devices[address]);
    g_devices[address] = nullptr;
    return err == ESP_OK;
}
